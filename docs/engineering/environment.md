...
- Ambiente **Producao**: o projeto inicial `mwgccjvztzbderlwtheg` (Oregon) foi removido vazio em 22/09/2026 e substituído por `CircuitoNE` (`ukyoyrmebwadmuzkswdw`, São Paulo). A proteção de revisão/branches foi verificada na preparação inicial; em 23/09/2026 UTC o App recebeu HTTP 403 ao tentar ler variables/secrets do environment. Atualizar e conferir a referência, URL e chaves antes de qualquer integração de produção; não presumir que o environment acompanha a troca automaticamente. Ver [#43](https://github.com/IgnisDevNE/CircuitoNE/issues/43).
