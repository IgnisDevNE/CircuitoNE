...
A suíte, suas propostas, o workflow de aceite e a referência ativa ficam sob autoridade externa à implementação. O repositório [IgnisDevNE/CircuitoNE-QA](https://github.com/IgnisDevNE/CircuitoNE-QA) foi criado na mesma organização e o mantenedor retirou-o da instalação do App `ignisdevne`. Não ampliar essa instalação para dar escrita no QA. O `main` do QA contém a primeira suíte de caracterização e o verificador; o CI próprio passou no commit `cd15b96`. A proteção de `main` exige o check `scripts` e uma aprovação independente. A branch `accepted` foi iniciada em `4f095a7`, vinculada ao `main` atual da aplicação.
...
O App `CircuitoNE QA Publisher` foi instalado apenas no CircuitoNE e no CircuitoNE-QA, com `Checks:write` e `Pull requests:write`. Cada token de execução limita separadamente repositório e permissão: publicação do check no CircuitoNE ou criação de PR de testes no QA. A chave foi colocada no environment protegido `QA Publisher` do QA; uma cópia local permanece temporariamente em `secrets/` até a limpeza de credenciais ao fim do housekeeping. A política da organização rejeitou habilitar criação de PRs pelo `GITHUB_TOKEN` padrão do QA (HTTP 409); o workflow usa esse token para preparar a branch e o App QA para abrir o PR, que o responsável aprova. O token de disparo, restrito ao QA com `Actions:write`, está no environment `QA Dispatch` do repositório principal e expira em 23/09/2027. O check exigido com origem fixada no App QA e os ensaios de aceitação/negação devem ser concluídos antes de ativar `CANONICAL_QA_ENABLED` ou encerrar a #31.
...
|---|---|---|
| Confirmar/provisionar QA e autoridade de publicação | Repositório QA, proteção de `main`, App QA e primeira referência criados; revogação das credenciais locais ainda pendente | App implementador não escreve QA, não troca a referência e não imita a identidade do aceite; controles independentes dos arquivos do PR |
