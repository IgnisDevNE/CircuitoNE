...
Estado em 23/09/2026: **operacional, com ensaio da automação pendente na [#58](https://github.com/IgnisDevNE/CircuitoNE/issues/58) e limites de isolamento na [#31](https://github.com/IgnisDevNE/CircuitoNE/issues/31)**. Aplica-se a código, testes, workflows, banco e infraestrutura do CircuitoNE. O [controle de acesso](access-control.md) define as identidades; a [ADR 0006](../decisions/0006-canonical-ci-suite.md) define a decisão; a [spec de aceite](../specs/canonical-ci-suite.md) define o contrato técnico. Este documento registra quem autoriza cada mudança e o que a esteira já executa.
...
As [QA PRs #12](https://github.com/IgnisDevNE/CircuitoNE-QA/pull/12) e [#13](https://github.com/IgnisDevNE/CircuitoNE-QA/pull/13) instalaram a retomada e o encerramento automático nas branches `main` e `accepted`. O CI do QA passou; após a integração, o workflow nas duas branches tem o mesmo objeto Git e a árvore ativa de testes continua igual ao registro em `.qa/state.json`. Falta observar uma aprovação de proposta e uma promoção reais com o novo fluxo antes de concluir a #58.
...
A [#58](https://github.com/IgnisDevNE/CircuitoNE/issues/58) acompanha o ensaio real da retomada automática e do fechamento pós-promoção. A aprovação humana dos testes e da PR principal continua obrigatória. O gatilho de revisão precisa existir tanto em `main` quanto em `accepted`, base das propostas; ele executa apenas o script confiável de `main`, sem código candidato nem segredos de ambiente.
