...
3. Contatos, presskit e cachê são acessíveis ao titular e aos administradores N2 com vínculo atual em coletivo aprovado. A criação de coletivo gera estado pendente: dashboard e funções internas ficam bloqueados até a aprovação pela administração do site, conforme RN-30.
...
- **Ambientes:** testes destrutivos em banco descartável; homologação no `CircuitoNE-dev`; produção separada e protegida. Previews usam dados sintéticos e não acessam produção. Configuração pública pode chegar ao cliente; segredos são injetados apenas no servidor.
