...
|---|---|---|
| F0-T14 | Node 22.23.2 via pnpm e container; TS 7.0.2; Tailwind/plugin 4.3.3; grupo Dependabot | Aplicação do patch de workflow e aceite GitHub; inspeção visual limitada |
...
| F0-T13 | LCOV/HTML/JSON; 4,33% statements, 4,58% linhas, 2,99% branches, 2,92% funções | Unidade somente; E2E não entra nesse percentual. Artefatos CI e upload Codecov pendentes |
| F0-T4 | CLI 2.117.0; adaptador com ordem/bytes/checksums; 2 testes de preparo; Linux com duas reconstruções de migração sintética temporária após o conjunto canônico vazio | Windows direto falhou; CI, schema/tipos/seeds reais e homologação compartilhada pendentes |
...
Revisores: subagentes `review_toolchain` e `review_environment_plan`, modelo Sol, esforço baixo, somente leitura; correções feitas pelo implementador. Achados corrigidos: incluir testes/configs no TypeScript; verificar que Ctrl-click preserva a ação nativa; corrigir o caminho do teste CSS inicialmente ignorado pelo Git; proteger o patch de workflow por CODEOWNERS; remover a pasta do ensaio de banco em cleanup; limitar a conclusão da auditoria Supabase à evidência coletada. Achado deferido: E2E/banco/artefatos fora do CI vigente, com patch concreto em `docs/engineering/ci-phase-zero.patch` e [#34](https://github.com/IgnisDevNE/CircuitoNE/issues/34)/[#32](https://github.com/IgnisDevNE/CircuitoNE/issues/32). Capturas atuais contêm só fixtures; revisar sanitização antes de capturar sessões/dados reais.

A revisão não transforma subagentes no verificador isolado exigido por F0-T2. O App não pode escrever workflows; nenhuma credencial humana foi usada para contornar essa restrição. O patch foi verificado com `git apply --check`, mas sua execução no GitHub depende do mantenedor. Revisão completa de saída será repetida quando F0-T7–T12 e os gates estiverem concluídos.
...
| A03 Cadeia de software: lockfile e digests; pnpm audit sem achados; workflow antigo | P2 | #34: aplicar action Node 24 e jobs E2E/banco; verificação local não substitui CI | Mantenedor | F0-T14; antes do aceite do checkpoint |
...
| [#34](https://github.com/IgnisDevNE/CircuitoNE/issues/34) | Mantenedor: aplicar patch concreto e validar action/E2E/artefatos no CI; F0-T14 |
